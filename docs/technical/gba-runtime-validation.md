...
狀態：第一版 framework 已實作；無版權 fixture 已驗證；兩款 ignored ROM 的最終 smoke
receipt 尚須完成後才能把整體工作標為 complete。
...
本機實測版本是 Homebrew mGBA 0.10.5。官方資料支持下列能力：
...
- terminator 必須在 allocated range 內；
...
- `breakpoint`／`watchpoint`：stop packet、PC/LR/register receipt；breakpoint 可依 register
  對 consumer source/destination 做 bounded hash capture；
...
- savestate 若只有需求文字、沒有 launch receipt，維持 unknown。
...
- 兩款 ignored ROM 的 framework-native smoke receipts；
- launch-time savestate receipt 的自動驗證（第一版只定義外層 provenance 邊界）；
