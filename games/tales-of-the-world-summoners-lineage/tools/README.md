...
## M33/M34 bounded target encoder／reinsertion POC
...
`m33_target_reinsertion_poc.py` 的 `m32` profile 接受 M32 caller literal `0x081FA4B4`，
`m34` profile 則只接受 M34 的 `0x08087384` source-pointer literal；兩者都把 bounded
target stream append 到 image end、只改寫一個 literal，重新讀取 terminator、檢查
unresolved unit 並輸出 source-free receipt。這是兩個實際 byte-changing relocation POC，
不是通用 encoder。target image／BPS 必須留在 `/private/tmp` 或 ignored work；BPS 建置與
套用使用 `core/patches/bps_create.rb`、`bps_apply.rb`。
...
M34 重跑方式（target 只是 official Latin bounded POC，並非 zh-TW glossary decision）：

```sh
PYTHONDONTWRITEBYTECODE=1 python3 \
  games/tales-of-the-world-summoners-lineage/tools/m33_target_reinsertion_poc.py \
  /private/tmp/project-atlantis-a9pj.gba --profile m34 --target-text 'Fulein' \
  --output /private/tmp/tow-a9pj-m34-reinsert/target.gba \
