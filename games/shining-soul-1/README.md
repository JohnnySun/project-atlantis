...
1. **最高優先**：定位「職業を選んでください」等 OBJ 句子用的假名字型表。第五輪已用逐字元 ROM 精確位元組搜尋確認這批字形未經壓縮、且不屬於任何已知表，假名集中在 ROM `0x46A000`–`0x46E000`（部分位址間距恰為 128 bytes 整數倍，暗示可能是另一張獨立、以 128-bytes-per-glyph 定址的表），漢字則分散在 `0x48xxxx`（見 `research/name-entry-hiragana-codepage.md`「附帶發現」一節）。下一步：收集更多 OBJ 句子（例如職業選擇畫面可能有多種職業名稱可切換）裡的假名字元位址，確認是否都落在同一範圍內、間距是否穩定為 128 的倍數，藉此反推表格起點與定址公式。
2. 補完 `research/name-entry-hiragana-codepage.md` 裡「低信心／未解讀」的符號類 tile-index（976、670、650、278、598、704，以及疑似重複的「あいうえお／ぁぃぅぇぉ」列）——目前只能從像素形狀猜測，需要找到實際使用這些 tile 的操作情境（例如真的在姓名輸入畫面打出小字或符號）才能核實。
3. 擴充 `research/bg-fonttable-codepage-partial.md`（`0x1398e8` 表）裡「低信心／僅結構性判讀」那些條目——tile 0–9、16–25 兩組額外數字樣式、tile 91–94 疑似 5–8、tile 176–191 疑似另一份 FILE+數字拷貝——目前都只從字型表圖片目測，沒有在任何實際畫面上看過被引用，需要找到會用到它們的畫面才能核實。
4. 找到字型搬移的通用「傳輸佇列」機制本身之後，回溯是哪段程式碼把 (來源=字型 ROM 位址, 目的=VRAM tile 槽, mode) 三元組寫進佇列 entry——本輪測試過的候選推入函式（ROM `0x08001154`）已用 150＋400 次即時攔截**否定**，不是字型使用的推入路徑，需要換一個候選或换一種找法（見上方「第三輪偵察」Goal 2 一節「未確認／已測試但否定的假設」）。這條路線優先度低於上面幾點，因為 tile-index 已確認直接對應 ROM 字型表位址，不再迫切需要靠這條呼叫鏈反推 codepage。
5. 仍未解決：對話文字（量遠大於 UI 提示字樣）究竟走這幾套已知字型表（BG ×2、OBJ 標題 ×1、OBJ 假名疑似另一張 ×1）的哪一套，還是完全獨立的第五套系統；目前確認的表都只涵蓋 UI／選單／輸入法用途，量體遠小於真正的劇情對白，不能假設兩者共用同一系統。文字究竟是 BIOS 壓縮、自訂壓縮、還是完全不壓縮直接存放也仍未解——目前所有已確認的字型資料都是不壓縮直接存放，但這仍只涵蓋固定 UI／輸入法圖塊，未觸及對話文字本身。姓名輸入畫面本身（打完名字按確定之後）會進入什麼畫面也還沒探索，可能是下一個推進方向。
