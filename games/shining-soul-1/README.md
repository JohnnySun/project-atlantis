...
2. 4 個與句子共用同一呼叫鏈、但未逐一驗證身分的 sprite 來源位址（角色圖示動畫疑似
   ...
   ——見 `research/obj-sentence-glyph-loader.md`「發現 2」。
...
6. 仍未解決：對話文字（量遠大於 UI 提示字樣）究竟走這幾套已知字型表（BG ×2、OBJ 標題 ×1、OBJ 主表 ×1）的哪一套，還是完全獨立的另一套系統；目前確認的表都只涵蓋 UI／選單／輸入法／單一畫面句子用途，量體遠小於真正的劇情對白，不能假設兩者共用同一系統。BG 系統（存檔選擇／姓名輸入畫面）是否也用本輪解出的「16-bit 代碼陣列」格式、還是維持第四／五輪已確認的「tilemap tile-index 即字符代碼」形式，本輪未實際驗證，推測是後者。姓名輸入畫面本身（打完名字按確定之後）會進入什麼畫面也還沒探索，可能是下一個推進方向。
