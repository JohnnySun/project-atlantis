...
1. **最高優先**：定位「職業を選んでください」這句話本身的位元組儲存位置——已知
   「index×`0x80`+base」能把字元代碼換算成字形 ROM 位址，但**哪裡儲存了「這格要畫
   index=44」這件事本身**（例如一個逐字元代碼陣列）仍未找到；本輪已測試多種假設（相鄰
   假名差值掃描、精確 Shift-JIS 搜尋、表基底 literal pool 搜尋）皆為負面結果，見
   `research/obj-sentence-glyph-loader.md`「未解決：句子本身的儲存位置」一節，避免
   重複已否定的路線。另可嘗試：在畫面切換瞬間對呼叫點 `0x080034d0` 前的迴圈本體
   （回推更早的呼叫者，或迴圈計數器 `r3` 所走訪的陣列本身）設 watchpoint，直接找出
   「要畫哪些 sprite」這份清單存在哪裡。
2. 漢字（職／業／選）定址機制仍未解——已確認**不在**主字形表的 `index*0x80` 定址規則內
   （三者對 `0x80` 取餘數不一致），可能是獨立指標或另一套（例如依 JIS 碼位）定址，本輪
   未深入。
3. 4 個與句子共用同一呼叫鏈、但未逐一驗證身分的 sprite 來源位址（角色圖示動畫疑似
   `0x084d26c4` 系列、`0x085df744`、疑似「剣士」兩字的 `0x08474904`／`0x08474984`）
   ——見 `research/obj-sentence-glyph-loader.md`「發現 2」。
4. enqueue 斷點測試腳本（對 `0x08001154` 雙 watchpoint 攔截約 680 次的那支）本輪只在
   `/tmp` 下臨時完成，未整理成 `tools/` 正式工具，留給下一輪（方法已完整記錄，重寫
   難度低）。
5. 補完 `research/name-entry-hiragana-codepage.md` 裡「低信心／未解讀」的符號類 tile-index（976、670、650、278、598、704，以及疑似重複的「あいうえお／ぁぃぅぇぉ」列）——目前只能從像素形狀猜測，需要找到實際使用這些 tile 的操作情境（例如真的在姓名輸入畫面打出小字或符號）才能核實。
6. 擴充 `research/bg-fonttable-codepage-partial.md`（`0x1398e8` 表）裡「低信心／僅結構性判讀」那些條目——tile 0–9、16–25 兩組額外數字樣式、tile 91–94 疑似 5–8、tile 176–191 疑似另一份 FILE+數字拷貝——目前都只從字型表圖片目測，沒有在任何實際畫面上看過被引用，需要找到會用到它們的畫面才能核實。
7. 仍未解決：對話文字（量遠大於 UI 提示字樣）究竟走這幾套已知字型表（BG ×2、OBJ 標題 ×1、OBJ 主表 ×1）的哪一套，還是完全獨立的另一套系統；目前確認的表都只涵蓋 UI／選單／輸入法／單一畫面句子用途，量體遠小於真正的劇情對白，不能假設兩者共用同一系統。文字究竟是 BIOS 壓縮、自訂壓縮、還是完全不壓縮直接存放也仍未完全解——目前所有已確認的字型資料都是不壓縮直接存放，但句子本身的「字元代碼序列」儲存格式仍未知（見上方第 1 項）。姓名輸入畫面本身（打完名字按確定之後）會進入什麼畫面也還沒探索，可能是下一個推進方向。
