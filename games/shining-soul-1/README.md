...
- **category 5–15 容量擴充**（暫緩，非放棄）：第十五輪的中文字形插入 proof of concept 只能用 category 4 的 215 個空格位，遠不夠一整套翻譯。真正的擴充路徑（把 IWRAM 查表初始化程式碼補上、指向 ROM `0x660000+` 空白區的新表）連續三輪（16–18）卡住無進展，方向已改成不需要模擬器的工作，見 `SESSION-LOG.md`「第十六至十九輪」一節與 `research/obj-sentence-category4-and-dispatch-table.md`「可規模化設計需要什麼」。
...
第十五輪已證實可以把翻譯文字寫回 ROM 並在遊戲裡正常渲染：插入 2 個中文字形（GNU Unifont 轉換）＋改寫一則已知字串，mGBA 實機渲染確認「道具」正確顯示、相鄰內容不受影響。這只是可行性驗證，容量（215 格）與字串變長（本遊戲沒有指標表，搬動字串位置比黃金太陽困難）都還沒有可規模化的解法。見 `SESSION-LOG.md` 第十五輪、`tools/build_cn_glyph_poc.py`。
