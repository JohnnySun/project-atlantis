...
1. 漢字（職／業／選／色）的 `glyph_entry_index` 定址機制仍未解——已知它們的字串代碼
   `category` 是 2 或 3（不同於假名的 0），但漢字像素資料本身的 ROM 位置、`category`
   欄位的完整語意（本輪只看過 0/2/3 三種值，機制上似乎支援到 16 種）都還沒找到，見
   `research/obj-sentence-string-format.md`「未解決」一節。
...
3. `0x499000` 附近區域是否為全域字串池、範圍多大、有無指標表指向各字串——本輪只確認了
   兩個具體句子的位址，未系統性掃描這個區域。
