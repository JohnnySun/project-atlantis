...
- [x] 導入共用 `scripts/gba-rom-identity.py` 與 `scripts/gba-runtime-session.py`：M5.5 target identity exit `0`／status `pass`，manifest strict validation exit `0`，port `2351` preflight/run 均 exit `0`；session ownership pass 且 PID `35308` 安全清理，generic runtime report 只確認自然 palette queue `0x03005D60→0x05000000`／`0x400` 與 hash-only capture。共用 pass 不升格為文字 consumer／畫面 QA；本作 handler／codepage／glyph adapter 保留。
