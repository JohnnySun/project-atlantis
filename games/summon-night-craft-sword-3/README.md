...
- M2.8 以固定 ROM hash 重抽 13 個 resource／361 筆 record，確認 11 個 pointer payload groups、resource `9`／`10` 的 zero-span alias、positive span 不重疊、361/361 source re-encode 與 record-contract aggregate SHA-256；line/page/wait、glyph width 與 translated multi-resource insertion 仍 unknown，semantic no-op container rebuild 另有 M5 收據。
...
- M5.5 以 resource 24 的第三筆相同 prize header `b3cj:t2:024:0x01f0` 建立第八筆 cumulative target；沿用 `ec64/ec65/ec66→0x847/0x848/0x849`，不新增 glyph，resource compressed 維持 `1406`，361 筆重抽為 target `8`／untouched `353`，target ROM SHA-256 `acfb3587a8217bf4ea444daf25f32c0947998a9203ee874db5006d7b6b016db6`、CRC32 `fc874c4d`，BPS `4856` bytes apply byte-identical。這仍是 static-only；八筆現為 `ai_review`，不是人工核准、runtime QA 或發布 patch。
- M5.5 後已完成八筆 bounded consistency／字型／單行版面 review；八筆維持 provisional wording，`重金礦` 已由兩個臺灣社群來源交叉支持但仍待人工術語核准，ledger 提升為 `ai_review`，但仍待外部人工術語核准。這是可審核的 session review，不等同 `approved` 定稿。
- M5.5 後的 fixed-port SDL route 在 port `2346` bind 前失敗；後續 static audit v3 以 13 個 function／16 個 literal guards 另確認 `sub_08009654` 的 `0x20`-byte text tile-data address formula 與 `sub_08001BC0`→`sub_080092CC` 的 OAM buffer `0x030038B0`→`0x07000000` copy path，同時保留 writer→DMA queue→`0x06010000` 與 palette shadow→`0x05000000` 證據；tilemap、live cache/OAM/VRAM、自然 reachability 與畫面可讀性仍 unknown。完整收據見 [`research/m5.5-release-gates.md`](research/m5.5-release-gates.md)。
- M5.5 後以 `tools/rebuild_container.py` 對 13 resources／11 payload groups 做 semantic PSI3／LZ77 no-op rebuild：361 records、source re-encode、directory、capacity guard 與 BPS apply 均通過；這是 container engineering evidence，不是 translated multi-resource release encoder。完整收據見 [`research/m5-container-rebuild.md`](research/m5-container-rebuild.md)。
- M5.5 後以 `tools/rebuild_full_container.py` 擴展 semantic no-op coverage 到固定 type-2 table 全部 79 entries／68 non-zero PSI3 payload groups：zero-span alias clusters、68/68 span guards、79-entry PSI3 round-trip、361/361 source re-encode、directory byte identity 與 `24507` bytes payload-only diff 均通過；BPS `26147` bytes apply byte-identical。這仍不是 translated release encoder；完整收據見 [`research/m5-full-container.md`](research/m5-full-container.md)。
- M5.5 後以固定 mGBA clean clone 的 compile-time port `2348` 做本作專屬 runtime retry：自有 process 曾完成一次 listener／`qSupported`／`S02`，但 controlled IWRAM scratch write 回 `E06`，後續 direct AF_INET／same-launcher probe 回 `Errno 49`；沒有 text consumer／writer／VRAM 或畫面證據。完整 transport chronology 見 [`research/m5-runtime-text-probe.md`](research/m5-runtime-text-probe.md)。
- M5.5 後以固定 clean mGBA HEAD 的 compile-time port `2350` 完成一次 fresh single-connection runtime probe：PID `91895`、qSupported exact／`S02`、自然 palette queue `0x03005d60→0x05000000`／`0x400`，再以明確 controlled `ec64 ec65 ec66 0000` 命中 `sub_080036F8`／`sub_08002CB4` 的 `0x847/0x848/0x849`，受控 queue／flush 後 text VRAM `0x06010000` 非零 131 bytes，三個 writer／VRAM slice hash 對上；adjacent physical `0x846` 為全零。palette readback 全零、BG0–3 全零、32×24 static render 全黑，因此這是 controlled writer→VRAM 證據，不是自然 consumer／畫面可讀性通過；完整收據見 [`research/m5.5-runtime-writer-qa.md`](research/m5.5-runtime-writer-qa.md)。
- M5.5 後以固定 clean mGBA HEAD 的 compile-time port `2351` 完成第二份 fresh single-connection runtime receipt：PID `19669`、qSupported／`S02`、自然 palette queue 後，只寫 CPU `lr/pc` 受控進入 `0x0800D81C`；第一個 downstream stop 為 `sub_0800B730`，再 bounded 命中 `sub_080036F8 → sub_08002CB4 × 6`。boot script context current-pointer 為零，return sentinel 未到達；這證實 handler 的受控下游 callsite，不是自然 target record、VRAM／palette／tilemap／OAM 或畫面 QA。hash-only probe 與 test 見 [`tools/runtime_m5_handler_probe.py`](tools/runtime_m5_handler_probe.py)、[`research/m5.5-runtime-writer-qa.md`](research/m5.5-runtime-writer-qa.md)。
- M5.5 後導入共用 ROM identity／runtime-session：target identity、manifest、2351 preflight 與 ownership-managed run 均 exit `0`／status `pass`；PID `35308` 由共用 identity guard 安全 terminated，generic runtime report 只證實自然 palette queue `0x03005D60→0x05000000`／`0x400` 與 hash-only font/palette/state capture。這個共用 pass 不代表文字 consumer、target glyph VRAM、tilemap／OAM、畫面可讀性或發布；命令與報告 hash 見 [`research/m5.5-release-gates.md`](research/m5.5-release-gates.md)，manifest 見 [`research/m5.5-runtime-session-case.json`](research/m5.5-runtime-session-case.json)。
- M5.5 後以 `tools/audit_static_text_oam.py` 補證 `sub_0800D81C → sub_0800B730` 的 per-glyph object descriptor、`sub_08001C00 → sub_0800901C` 的 `gOamBuffer=0x030038B0` pack，以及既有 `0x07000000` OAM DMA；這仍是 static control-flow proof，不是 live OAM／tilemap／畫面 QA。完整收據見 [`research/m5-static-text-oam.md`](research/m5-static-text-oam.md)。
- 相同 byte length 的 record-level 原地修改可行；zero padding 縮短 blocked，變長需 resource rebuild；未知 VM、字型 width／tilemap、全遊戲 translated LZ77／pointer encoder 與 live runtime QA 仍待建立。
- 至少八個有限量 target 通過各自的 `restore → work → strip`／source hash contract 與 repository safety check；M3 validator 已重驗 source／work／ledger 分界，但八筆現為 `ai_review`，仍需外部人工定稿與 runtime review。
- 編碼器／回插器拒絕來源 hash、缺字、控制碼或長度不一致，而不是放寬檢查。
- 重建 ROM 重新抽取吻合；BPS round-trip 與 mGBA 核心畫面回歸另有收據。

目前已完成 clean 日版 ROM 身分／hash、M1.5 靜態 decoder、M2.1 控制碼保真 parser、361 筆 ignored source extraction、M2.2 static font chain／POC、M2.3 fail-closed allocation／bounded POC、M2.5／M4.1／M4.2／M4.3／M5.2／M5.3／M5.4／M5.5 cumulative `ai_review` zh-TW static build／BPS round-trip、M2.8 pointer／record contract audit、M3 ledger restore-strip validator，以及 M2.6 target／adjacent static proof、M2.7／M5.5 transport diagnostics、static writer chain audit 與 M5.5 後 clean 2350 controlled writer→VRAM receipt；尚無人工翻譯定稿、完整 VM／multi-resource release encoder、自然畫面 runtime QA 或可發布 patch 收據。
