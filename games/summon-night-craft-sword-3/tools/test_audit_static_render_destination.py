...
class StaticRenderDestinationTest(unittest.TestCase):
    ...
    def test_writer_dma_and_vram_chain_is_locally_hash_guarded(self) -> None:
        ...
        self.assertEqual(report["evidence_level"], "confirmed-static-writer-dma-vram-palette-hardware-copy")
        self.assertEqual(len(report["function_checks"]), 10)
        self.assertEqual(len(report["literal_checks"]), 10)
    ...
    def test_palette_hardware_destination_is_static_and_tilemap_oam_remain_unknown(self) -> None:
        self.require_rom()
        report = AUDIT.audit_rom(ROM_PATH)
        ...
        self.assertEqual(report["oam"]["destination"], "unknown")
