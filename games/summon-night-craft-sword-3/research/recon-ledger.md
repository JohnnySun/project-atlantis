...
| `REVIEW-001` | M5.5 後既有八筆 target consistency review | `confirmed`（bounded static metadata）／`provisional`（人工語意／術語） | [`research/m5-static-review.md`](m5-static-review.md) 逐筆固定八個 stable ID、target hash、單行／byte-length、control／glyph／adjacent contract；八筆 wording 可維持 provisional，`重金礦` 已由兩個臺灣社群來源確認來源名但仍待人工核准，全部仍為 `ai_draft` | 不新增同型 target；需人工語意／術語／字型／版面核准後才可改 status |
...
| `RUNTIME-009` | M5.5 clean clone 2347 runtime transport／palette live call | `confirmed`（transport／palette callsite）／`blocked`（text path） | clean clone source `26b7884bc25a5933960f3cdcd98bac1ae14d42e2`；compile-time port `2347`；binary SHA-256 `b4488dc0ffad295ce97b579a4b1da543ddd7e4aac8a457f64aa00c58b49fbc17`；fresh PID `78339`、single core client connection、`qSupported`／`S02` success；12 bounded stops included six `sub_08010CD4` and six `sub_08006BA4` hits with live `r0=0x03005d60`、`r1=0x05000000`、`r2=0x400`；`sub_0800D084`／`sub_080036F8`／`sub_08002CB4`／`sub_0800B730` not hit；one later controlled attempt failed before packet with `Errno 49` | Confirms runtime transport and palette shadow→hardware destination call only. No controlled scratch write, target consumer/writer, `0x847/0x848/0x849` live VRAM, adjacent live equality, tilemap/OAM, or screen readability; full receipt in [`research/m5.5-runtime-qa.md`](m5.5-runtime-qa.md) |
| `RUNTIME-010` | clean 2347 text consumer probe with controlled KEYINPUT | `confirmed`（transport／input writes）／`blocked`（text path） | [`research/m5-runtime-text-probe.md`](m5-runtime-text-probe.md) 記錄 fresh process、`localhost` single connection、qSupported／`S02`、四個 local text breakpoints、10 個 interrupt snapshots 與 active-low `0x03ff/0x03fe/0x03f7/0x03f6` KEYINPUT writes；沒有文字 breakpoint hit；VRAM `0x06000000`、palette `0x05000000`、OAM `0x07000000`、IWRAM `0x03000000` read 均回 `E06` | 這是明確 controlled input／transport evidence，不是 natural reachability；沒有 target consumer/writer、font cache、`0x847/0x848/0x849`／adjacent `0x846` live bytes、tilemap/OAM state 或 screen readability |
| `RUNTIME-011` | clean 2348 build／single-listener readiness 與 controlled memory boundary | `confirmed`（clean bind／qSupported）／`blocked`（memory injection／text path） | 固定 mGBA HEAD `26b7884bc25a5933960f3cdcd98bac1ae14d42e2` 的乾淨 clone 只改 compile-time port `2348`；binary SHA-256 `fe11732d1686b66bb5304cdbe13636e7e9daf648196e13c2b5f110e71841cc9`；本 session 自有 PID `26158`／`28984` 觀察到 listener，首個 single connection 完成 qSupported exact／`S02`，但 controlled zero-only IWRAM `M 0x03007000` 回 `E06`；第二次 direct AF_INET 與 same-launcher alternative 在首 packet 前回 `Errno 49`，均已清理 | 只確認 clean listener／一次 readiness，不是 text consumer／writer hit；`0x847/0x848/0x849`、adjacent `0x846` live VRAM、cache、palette readback、tilemap/OAM 與 screen readability 仍 unknown；不得重複同一 IWRAM injection／bind method，八筆 ledger 維持 `ai_draft` |
...
| `RENDER-003` | static text-object → OAM buffer → hardware OAM chain | `confirmed`（static）／`blocked`（live） | [`tools/audit_static_text_oam.py`](../tools/audit_static_text_oam.py) 先重跑既有 13-function／16-literal writer/DMA guards，再以 `sub_0800B730` full text-object range SHA-256 `59feabb18a62ea301bb7d453dca387fda15076a9f4a8a5b8758a56e73b37df38`、`sub_0800901C` SHA-256 `e6d1f338dfb6acf124f197e77f14460a5563480d301900e973522c00b98272b1` 與 5 個 local OAM literal guards 證實 per-glyph descriptor stride `0x28`、`gOamBuffer=0x030038b0`、main-loop OAM pack、既有 `0x07000000` DMA | 證實 text-object metadata 的 static OAM path，不是 live OAM values／changed glyph visibility；tilemap、VRAM readback、palette readback、natural reachability 與 screen readability 仍 unknown |
| `RENDER-003` | static text tile address／OAM DMA callsite chain | `confirmed`（static）／`blocked`（live） | audit v3 以 13 個 local function hashes、16 個 literal guards、csm3 commit `7e388ac` 交叉驗證：`sub_08001BC0` → local `sub_080092CC` 將 `0x030038B0` 以 DMA `0x84000100`／`0x400` bytes 複製到 `0x07000000`；`sub_08009654` 證實 `(((var->unk2 & 0x3ff)+var3->unk2)*0x20)+0x06010000`、mask `0x3ff`、table root `0x030040C0`；ignored report SHA-256 `e38a2e8a5bc7d7ee10c41321132d01b42bef03c8f3d817be6afbe0fbfa5598ec` | OAM transfer／tile-data addressing 是 static proof，不是文字 glyph 使用 OAM、tilemap placement、live OAM/VRAM、screen readability 或 runtime QA；tilemap 維持 unknown |
| `CONTAINER-001` | 13 resources／11 payload groups semantic no-op LZ77／PSI3 rebuild | `confirmed`（static） | [`tools/rebuild_container.py`](../tools/rebuild_container.py) 對固定 B3CJ 13 IDs 重發 PSI3 opaque／known token、deterministic LZ77 recompress、原 pointer span zero-fill；361/361 record identity、32092 stream bytes、source re-encode、directory unchanged、8191 changed bytes 全在 payload spans；rebuilt ROM SHA-256 `badb5838efafc3eb585c271208281bf44ce120ab46ec1ad573b4ddd55d185793`、CRC32 `5344e805`；BPS `8615` bytes、SHA-256 `326217ae194287d394fda8ef0296e095dff378d0c034238a48cae9c848080034`、apply byte-identical | 這是 semantic container encoder／verifier，不是翻譯；未知 VM semantics、全遊戲變長 translation policy、live runtime、tilemap／screen 與發布 patch 仍未證實；完整摘要見 [`research/m5-container-rebuild.md`](m5-container-rebuild.md) |
| `CONTAINER-002` | 全 type-2 table semantic no-op LZ77／PSI3 rebuild | `confirmed`（static） | [`tools/rebuild_full_container.py`](../tools/rebuild_full_container.py) 枚舉 79 entries、68 non-zero payload groups 與 11 zero-span entries；三個 alias clusters 保留不獨立寫入；79-entry PSI3 round-trip、361/361 source record re-encode、68/68 capacity guards、directory unchanged、24507-byte payload-only diff 與 BPS `26147` bytes apply byte-identical；report aggregate SHA-256 `6692fc24b14d9b225edaf7484a2d977af7e2e4a46580ca5be8313b3c5a9cf705` | 只關閉完整 type-2 semantic no-op coverage，不是 translated release encoder；變長 target、未知 VM/layout、general relocation、runtime screen、header／發布 policy 仍未證實；完整摘要見 [`research/m5-full-container.md`](m5-full-container.md) |
| `REPO-001` | 本作 path-limited milestone commit | `resolved` | 歷史 index-lock permission negative；後續以 path-limited commit `87ed08a14a07e900c7593456b1c42f2ea400f792` 成功提交，name-only scope 只有 `games/summon-night-craft-sword-3/**`，作者 `JohnnySun <bmy001@gmail.com>` | 後續每一個 commit 仍須重新核對 staged scope；不使用 alternate index、commit-tree、廣泛 stage、reset、checkout 或 stash |
