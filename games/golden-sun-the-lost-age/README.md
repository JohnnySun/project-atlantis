...
- **2026-08-15**：依 Wikipedia／社群 wiki 交叉核對，將メアリィ（Mia）、シバ（Sheba）的譯名分別由「米雅」「席芭」統一為「米雅莉」「西芭」（140 筆記錄，13 個翻譯檔案；詳見 `translations/glossary.zh-TW.tsv` 對應條目的日期註記）。**此變更晚於上述試作構建，尚未重新構建與驗證** — 下一次構建前必須先重跑 `tools/build_zh_tw_trial.rb` 並重做 `verify_text_delta.rb`／BPS 往返校驗，不可沿用舊構建產物。
