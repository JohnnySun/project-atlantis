...
- 已用日版原字形和完整碼表確認首批系統訊息、序章對話及德里村民對話，建立 745 條 `zh-Hans`／`zh-TW` 可審核草稿。
- 已完成資料驅動的 `zh-TW` 技術試作：從多個翻譯 JSONL 重建全套 Huffman 資料、加入 967 個繁體中文字形，並替換 745 條訊息。
...
目前替換 745 條開機、存檔、資料繼承、密碼轉移、難度選擇、姓名輸入、戰鬥、基礎選單、設定介面、商店狀態、戰鬥效果、插值戰鬥訊息、四元素精靈效果、召喚效果說明、序章燈塔出口、拉利貝洛守衛分支、撤離路線、燈塔碑文、半島會合、船隻調查、漂流、生還者、海嘯、靠岸、尋人入隊、亞歷克斯使命、席芭命運分支及德里村海嘯後與失蹤孩童對話；以下列出代表項目，完整資料見 `translations/*.draft.jsonl`，首版術語見 `translations/glossary.zh-TW.tsv`：
...
| 6179–6219 | 德里村海嘯後對話 | 村莊與印德拉大陸介紹、海嘯災情、毀損船隻、海神祠傳聞，以及失蹤孩童庫普爾與貝查羅的家人對話 |
...
目前的試作資料從 `0xF80000` 寫入 281,856 bytes，指標改為：

| 項目 | 新 GBA pointer | ROM offset |
| --- | ---: | ---: |
| 擴展字型 | `0x08F80000` | `0xF80000` |
| Huffman 表 | `0x08FC4B40` | `0xFC4B40` |
| 文字表 | `0x08FC4B70` | `0xFC4B70` |

新增字形 ID 已到 `0x55E`，因此構建器使用六組上下文 Huffman 樹；通用抽取器已從六組樹完整反解全部 12,772 條訊息。
...
- 基準 CRC32：`830b795f`
- 試作 CRC32：`c6748db6`
- BPS patch CRC32：`6645f627`
- BPS 大小：283,443 bytes
- 試作與重套 ROM SHA-256：`c8ec8e26964d5072e86bed9539fd6d92c2eeaa75a30c53056f7e9a3d844489c3`

用新指標重新抽取後，只有二十個翻譯批次指定的 745 個 ID 不同；其餘 12,027 條訊息的 12-bit 代碼序列與來源 TSV 完全一致。構建器會先用碼表反解並核對每筆翻譯記錄的日文原文，避免人工辨識錯誤直接進入 ROM。
