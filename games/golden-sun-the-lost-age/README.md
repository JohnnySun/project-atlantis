...
- 已用日版原字形和完整碼表確認首批系統訊息及序章對話，建立 323 條 `zh-Hans`／`zh-TW` 可審核草稿。
- 已完成資料驅動的 `zh-TW` 技術試作：從多個翻譯 JSONL 重建全套 Huffman 資料、加入 589 個繁體中文字形，並替換 323 條訊息。
...
目前替換 323 條開機、存檔、資料繼承、密碼轉移、難度選擇、姓名輸入、戰鬥、基礎選單、設定介面、商店狀態、戰鬥效果、插值戰鬥訊息、四元素精靈效果、召喚效果說明與序章首段對話；以下列出代表項目，完整資料見 `translations/*.draft.jsonl`，首版術語見 `translations/glossary.zh-TW.tsv`：
...
| 5761–5794 | 維納斯燈塔序章 | `我們真的可以／就這樣離開嗎？`；保留加西亞、潔絲敏與席芭的動態姓名插值 |
...
目前的試作資料從 `0xF80000` 寫入 268,928 bytes，指標改為：

| 項目 | 新 GBA pointer | ROM offset |
| --- | ---: | ---: |
| 擴展字型 | `0x08F80000` | `0xF80000` |
| Huffman 表 | `0x08FC18D0` | `0xFC18D0` |
| 文字表 | `0x08FC18F0` | `0xFC18F0` |

新增字形 ID 已到 `0x3E4`，因此構建器使用四組上下文 Huffman 樹；通用抽取器已從四組樹完整反解全部 12,772 條訊息。
...
- 試作 CRC32：`1e02a990`
- BPS CRC32：`2144df1c`
- BPS 大小：270,460 bytes
- 試作與重套 ROM SHA-256：`75456a1fa200954a41d63d4b8567ca99f9d1f2a77a5a9c7f4ba59b3c4a92c4db`

用新指標重新抽取後，只有八個翻譯批次指定的 323 個 ID 不同；其餘 12,449 條訊息的 12-bit 代碼序列與來源 TSV 完全一致。構建器會先用碼表反解並核對每筆翻譯記錄的日文原文，避免人工辨識錯誤直接進入 ROM。
