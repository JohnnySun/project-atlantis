...
- 已用日版原字形和完整碼表確認首批系統訊息、序章對話、德里村民、坎德拉寺及海神祠孩童救援對話，建立 912 條 `zh-Hans`／`zh-TW` 可審核草稿。
- 已完成資料驅動的 `zh-TW` 技術試作：從多個翻譯 JSONL 重建全套 Huffman 資料、加入 1,112 個繁體中文字形，並替換 912 條訊息。
...
目前替換 912 條開機、存檔、資料繼承、密碼轉移、難度選擇、姓名輸入、戰鬥、基礎選單、設定介面、商店狀態、戰鬥效果、插值戰鬥訊息、四元素精靈效果、召喚效果說明、序章燈塔出口、拉利貝洛守衛分支、撤離路線、燈塔碑文、半島會合、船隻調查、漂流、生還者、海嘯、靠岸、尋人入隊、亞歷克斯使命、席芭命運分支、德里村、坎德拉寺，以及海神祠失蹤孩童的繩索救援、會合與返村對話；以下列出代表項目，完整資料見 `translations/*.draft.jsonl`，首版術語見 `translations/glossary.zh-TW.tsv`：
...
目前的試作資料從 `0xF80000` 寫入 286,380 bytes，指標改為：

| 項目 | 新 GBA pointer | ROM offset |
| --- | ---: | ---: |
| 擴展字型 | `0x08F80000` | `0xF80000` |
| Huffman 表 | `0x08FC5CEC` | `0xFC5CEC` |
| 文字表 | `0x08FC5D1C` | `0xFC5D1C` |

新增字形 ID 已到 `0x5EF`，因此構建器使用六組上下文 Huffman 樹；通用抽取器已從六組樹完整反解全部 12,772 條訊息。
...
- 基準 CRC32：`830b795f`
- 試作 CRC32：`b9223966`
- BPS patch CRC32：`9e8053e3`
- BPS 大小：287,687 bytes
- 試作與重套 ROM SHA-256：`fceb7e79bcfd8274a5b89d3bf0cf22bf1ef4743bebbd92d670c5102024de6cb6`

用新指標重新抽取後，只有三十一個翻譯批次指定的 912 個 ID 不同；其餘 11,860 條訊息的 12-bit 代碼序列與來源 TSV 完全一致。構建器會先用碼表反解並核對每筆翻譯記錄的日文原文，避免人工辨識錯誤直接進入 ROM。
