#!/usr/bin/env python3
"""Bounded A5TJ demon-record code-unit cross-map.

M1.30 follows the independently named ``0x080bf648`` accessor and its
``0x080e1644`` caller to a small prefix of the ROM record family at
``0x0819cb74``.  Sixteen public Japanese demon-name anchors are compared
privately against the fixed ``+0x22`` eight-halfword field.  The report emits
only addresses, boundaries, hashes, lengths, counts, reference IDs, and match
metadata; it never emits record bytes, unit values, decoded text, glyphs, or a
translation ledger.

The table extent is intentionally not inferred from this prefix.  In
particular, the accessor has special branches for indices outside this probe,
so this tool does not turn those branches into a record-count claim.
"""
...
from m111_obj_consumer import _boundary_metadata, _direct_bl_callers_index  # noqa: E402
from m19_state_mapping import _function_end  # noqa: E402


SCHEMA = "smt2.m1.30.demon-crossmap.v1"

ACCESSOR = 0x080BF648
ACCESSOR_END = 0x080BF6CE
ACCESSOR_LITERAL_INSTRUCTION = 0x080BF656
TABLE_BASE = 0x0819CB74
TABLE_STRIDE = 0x60
FIELD_OFFSET = 0x22
FIELD_UNIT_COUNT = 8

CONSUMER = 0x080E1644
CONSUMER_END = 0x080E17F2
ACCESSOR_CALLSITE = 0x080E1746
LARGE_READER_CALLSITE = 0x080E17A6
SMALL_READER_CALLSITE = 0x080E17CC
LARGE_READER = 0x080AC3AC
SMALL_READER = 0x080AC334
OBJECT_ID_ARRAY_OFFSET = 0x26
OBJECT_ID_SLOT_COUNT = 5
CONSUMER_FIELD_OFFSET = FIELD_OFFSET
STACK_BUFFER_OFFSET = 0x0C

# Public Japanese names are comparison anchors only.  They are deliberately
# kept to a small contiguous prefix and are not emitted by the report.
_ANCHORS = (
    ("demon-sequence-00", "サタン"),
    ("demon-sequence-01", "メタトロン"),
    ("demon-sequence-02", "カマエル"),
    ("demon-sequence-03", "サリエル"),
    ("demon-sequence-04", "クシエル"),
    ("demon-sequence-05", "オファニム"),
    ("demon-sequence-06", "ラグエル"),
    ("demon-sequence-07", "ハニエル"),
    ("demon-sequence-08", "ザフキエル"),
    ("demon-sequence-09", "ラミエル"),
    ("demon-sequence-10", "アマテラス"),
    ("demon-sequence-11", "ツクヨミ"),
    ("demon-sequence-12", "タケミカヅチ"),
    ("demon-sequence-13", "ヒノカグツチ"),
    ("demon-sequence-14", "オモイカネ"),
    ("demon-sequence-15", "タヂカラオ"),
)

# Keep only the units required by this bounded demon anchor set.  This is not
# a complete codepage and is never serialized into the report.
_DEMON_UNIT_MAP = {
    0x017B: "ア",
    0x017A: "ァ",
    0x017D: "イ",
...
REFERENCE_SOURCES = (
    "https://w.atwiki.jp/shinmegamitensei2/pages/385.html",
    ...
    "https://kaerukyo.net/dds_database/devil.php?title=2",
)
...
def _boundary(data: bytes, entry: int, expected_end: int) -> dict[str, object]:
    ...
    return {
        ...
        "prologue_is_thumb_push_lr": _safe_u16(data, entry) is not None
        and (_safe_u16(data, entry) & 0xFF00) == 0xB500,
        "return_pop_r1": _safe_u16(data, expected_end - 4) == 0xBC02,
        "return_bx_r1": _safe_u16(data, expected_end - 2) == 0x4708,
        "return_bx_r0": _safe_u16(data, expected_end - 2) == 0x4700,
    }
...
def _field_units(data: bytes, ordinal: int) -> tuple[list[int], str, int]:
    field_address = TABLE_BASE + ordinal * TABLE_STRIDE + FIELD_OFFSET
...
def _anchor(data: bytes, ordinal: int, reference_id: str, expected: str) -> dict[str, object]:
    ...
    decoded = "".join(_DEMON_UNIT_MAP.get(unit, "") for unit in units)
    mapped = len(decoded) == len(units) and all(unit in _DEMON_UNIT_MAP for unit in units)
    return {
        "ordinal": ordinal,
        "stable_id": f"m30-demon-record-{ordinal:04d}",
...
def static_report(data: bytes) -> dict[str, object]:
    ...
    manifest = json.dumps(
        {"units": sorted((f"{unit:04x}", char) for unit, char in _DEMON_UNIT_MAP.items())},
        ensure_ascii=False,
        sort_keys=True,
    ).encode("utf-8")
    ...
    callsites = [
        ...
        _call_metadata(data, LARGE_READER_CALLSITE, LARGE_READER),
        _call_metadata(data, SMALL_READER_CALLSITE, SMALL_READER),
    ]
    ...
    return {
        ...
        "scan_scope": {
            "method": "sixteen_consecutive_demon_name_anchors_at_named_accessor_target",
        ...
        "static_provenance": {
            ...
            "callsites": callsites,
            "direct_bl_callers": direct_callers_meta,
            "accessor_contract": {
                "index_scale": "index * 0x60",
                "returned_record_field_offset": FIELD_OFFSET,
                "special_indices_outside_probe": ["0x00000106", "0x00000109", "0x0000010c"],
                "special_index_extent_claimed": False,
            },
            "consumer_contract": {
                "object_id_array_offset": OBJECT_ID_ARRAY_OFFSET,
                "slot_count": OBJECT_ID_SLOT_COUNT,
                "accessor_callsite": hex_address(ACCESSOR_CALLSITE),
                "copied_unit_count": FIELD_UNIT_COUNT,
                "stack_buffer_offset": STACK_BUFFER_OFFSET,
                "appended_terminator": "0x0000",
                "reader_targets": [hex_address(SMALL_READER), hex_address(LARGE_READER)],
            },
        },
        "category_crossmap": {
            "candidate_category": "demon",
            ...
            "stable_id_formula": "m30-demon-record-{ordinal:04d}",
            "identity_manifest_hash": hashlib.sha256(manifest).hexdigest(),
            "mapped_unit_count": len(_DEMON_UNIT_MAP),
        ...
        "conclusions": {
            "confirmed": (
                [
                    "accessor_literal_and_thumb_boundaries_match_named_demon_record_path",
                    "sixteen_consecutive_0x0819cb74_fields_match_external_demon_sequence",
                    "bounded_demon_code_unit_identity_is_separate_from_item_namespace",
                ]
                if static_confirmed
                else []
            ),
            "provisional": [
                "0x0819cb74_table_is_a_demon_record_family_for_bounded_prefix_only",
