...
## 2026-08-16：M1.28 control／layout fail-closed contract

本輪不新增翻譯、不重掃 pointer，重用 M1.23 的已驗證 consumer instruction window 與
M1.26 的 source-safe full-corpus join。`tools/m128_control_layout_contract.py` 會拒絕
reused report 的完整 `text`／`targets` key，並核對 ROM hash、NUL／glyph loop instruction
metadata、partition count 與 target-admissible count；`source` 這個欄位只有在 provenance／
address metadata 中允許，不能因此被命名成語意。

### 可重現結果

| 項目 | 結果 |
| --- | --- |
| proven byte boundary | source/render NUL exits；`ldrh` 2-byte unit；cursor `+2`；loop target；stack mode-field origin |
| newline | consumer window dedicated `0x0A/0x0D` compare `0`；opaque newline candidate `0`；全域 newline semantics 仍 `unconfirmed` |
| speaker／branch | mode-field origin proven；meaning／semantic label 均 `unconfirmed`／opaque，不命名 |
| width／line | observed `0..240px`；engine width limit `false`；single-line narrow token shape `939`；static POC cap `64px` |
| corpus | `2325/2325` source／NUL／token no-op；narrow `939`、mixed `833`、wide `417`、opaque `136`；opaque units `1120` |
| target boundary | encoder-admissible `12`；其餘依 M1.26 wide／missing／opaque／mixed contract reject；translation started `false` |

此 contract 只證明可機器判定的 byte／shape 邊界，不把沒有 dedicated compare 解讀成
「引擎沒有 newline」，也不把 mode-field 的數值路由命名成 speaker 或 branch。要解除
相應 reject，仍需要 producer/queue context 綁定欄位值，以及可用的 runtime stop protocol
與 screen／VRAM layout evidence。

## 2026-08-16：M1.29 caller／scene／wide release-boundary reconciliation

本輪停止新增 UI prompt，重用 M1.24 的 exact pointer／caller coverage、M1.21 的 wide
identity／resource capacity 與 M1.28 的 control/layout contract。工具
`tools/m129_scene_wide_release_boundary.py` 不重新掃描 pointer，也不把 structural
callsite class 改名成劇情語意；它只把「已知 caller 結構」和「scene semantics 尚未證明」
放進同一份 source-safe release matrix，並核對 wide existing identity 的 fail-closed policy。

### 可重現結果

| 項目 | 結果 |
| --- | --- |
| corpus/caller | `2325` records；`609` exact pointer candidates；`370` exact records；`123` cohorts；`5` verified direct consumer callsites |
| structural classes | wrapper fallback `1`；queue-entry drain `1`；dual-buffer UI `2`；indexed object buffer `1`；全數 semantic label `unconfirmed` |
| partition coverage | mixed `126/833` exact／`707` uncovered；narrow `83/939`／`856` uncovered；wide `101/417`／`316` uncovered；opaque `60/136`／`76` uncovered |
| scene matrix | story／branch／battle dialogue／unit-pilot-weapon-spirit／UI／speaker／newline 全 `unconfirmed`；natural caller／screen `not_observed` |
| wide identity | existing `743`；runtime-confirmed `1`；static-only `742` reject；new-slot capacity `0`；source-wide→target-narrow `147` occurrence edge 保留分欄 |
| release boundary | target encoder-admissible `12`；scene classification complete `false`；wide encoder complete `false`；translation started `false` |

wide 策略是「只重用有 bounded runtime identity 的既有 slot」，不是把 742 個 static-only
identity 或 0 個新槽轉成容量。下一步若 runtime transport 恢復，必須先將 exact source
record 綁定 producer/queue、mode field 與 screen/layout；在此之前保持所有 scene label
與 wide expansion `unconfirmed`／reject，不擴大全語料翻譯。
