...
- [x] M1.29 完成 structural caller coverage／scene semantics／wide strategy reconciliation：
  `2325` records、`609` exact pointer candidates、`370` exact records、`123` cohorts、
  `5` verified direct callsites 與 partition uncovered counts 重現；story／branch／battle／
  unit-pilot-weapon-spirit／UI／speaker／newline 全 `unconfirmed`，natural caller／screen
  `not_observed`。wide 只允許 existing runtime-confirmed identity：`743` existing、`1`
  runtime-confirmed、`742` static-only reject、new-slot `0`；source-wide→target-narrow
  `147` occurrence edge 保留分欄。摘要在
  [`research/m129-scene-wide-release-boundary.json`](research/m129-scene-wide-release-boundary.json)，
  工具／測試在 [`tools/m129_scene_wide_release_boundary.py`](tools/m129_scene_wide_release_boundary.py)
  與 [`tools/test_m129_scene_wide_release_boundary.py`](tools/test_m129_scene_wide_release_boundary.py)。
