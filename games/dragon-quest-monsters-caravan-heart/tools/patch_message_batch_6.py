#!/usr/bin/env python3
"""Patch a bounded clean A9HJ battle-start message.

This is a sixth fixed-span reinsertion proof for ``g06/v00/m0041``.  It
reuses four authored E1 glyphs from the first bounded menu proof, preserves
...
MESSAGE_STRING_ID = "dqmch:a9hj:g06:v00:m0041"
MESSAGE_FILE_OFFSET = 0x286765
MESSAGE_SPAN_LENGTH = 14
MESSAGE_DATA_LENGTH = 13
MESSAGE_EXPECTED_SHA256 = "a82c772728e2aabf385156c79a487797d67b5af06c234ab6069ff1b247e041c8"
SOURCE_HASH = "ad8d4b95a7b01a690488dcbe72109e703381f3be13dd28c21a9bc388820cd2f0"
TARGET_TEXT = "開始對戰。"
TARGET_TEXT_HANS = "开始对战。"
PRESERVED_TAIL = bytes.fromhex("ff")
ALT_LEAD = 0xE1
GLYPH_SLOTS = {character: MENU_GLYPH_SLOTS[character] for character in ("開", "始", "對", "戰")}
GLYPH_BITMAPS = {character: MENU_GLYPH_BITMAPS[character] for character in GLYPH_SLOTS}
DIRECT_CODES = {"。": 0x94}
...
def assert_allocations_unused(decoded: pathlib.Path) -> None:
    """Ensure the reused menu slots are unused in the clean extractor corpus."""
    ...
    for line in decoded.read_text(encoding="utf-8").splitlines():
        ...
        for token in record.get("tokens", []):
            if token.get("kind") == "alt-glyph" and token.get("lead") == ALT_LEAD and token.get("value") in allocated:
                raise ValueError(f"reused E1 slot is already used in clean corpus: 0x{int(token['value']):02X}")


def encode_target(text: str) -> bytes:
    if text != TARGET_TEXT:
        raise ValueError("batch 6 encoder expects the declared visible target")
    output = bytearray()
...
def patch(
    ...
    receipt = {
        ...
        "reused_e1_slots": {character: f"0x{index:02X}" for character, index in GLYPH_SLOTS.items()},
...
def main() -> int:
    ...
    try:
        ...
        print(f"patch_message_batch_6: {error}", file=sys.stderr)
        return 2
