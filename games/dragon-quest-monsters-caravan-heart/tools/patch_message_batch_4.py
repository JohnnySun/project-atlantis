#!/usr/bin/env python3
"""Patch a bounded clean A9HJ wait-message span.

This is a fourth fixed-span reinsertion proof for ``g06/v00/m0044``.  It
replaces one short wait prompt, preserves the final ``FF`` byte, and uses
three explicitly authored E1 glyph slots plus the clean direct full-stop
unit.  It is not the game's general encoder; ROM and generated outputs stay
local or ignored.
"""
...
MESSAGE_STRING_ID = "dqmch:a9hj:g06:v00:m0044"
MESSAGE_FILE_OFFSET = 0x286792
MESSAGE_SPAN_LENGTH = 16
MESSAGE_DATA_LENGTH = 15
MESSAGE_EXPECTED_SHA256 = "d32a41ec1d02fda1bc07417e1664228eb0e1a00e0d564f246c6c989a48be93dd"
SOURCE_HASH = "dc2e9dfdaaa0258b17314006966ad71d1da06bb5447e4c1a3c2ebbd3f21120c8"
TARGET_TEXT = "請稍候。"
TARGET_TEXT_HANS = "请稍候。"
PRESERVED_TAIL = bytes.fromhex("ff")
ALT_LEAD = 0xE1

# These slots are zero-use in the clean extractor corpus and disjoint from
# the three earlier bounded batches.
GLYPH_SLOTS = {"請": 0xF3, "稍": 0xF4, "候": 0xF5}
...
def validate_bitmaps() -> dict[str, bytes]:
    ...
    if set(GLYPH_SLOTS.values()) & RESERVED_BATCH_SLOTS:
        raise ValueError("batch 4 glyph slot overlaps an earlier bounded batch")
...
def main() -> int:
    ...
    try:
        ...
        print(f"patch_message_batch_4: {error}", file=sys.stderr)
        return 2
