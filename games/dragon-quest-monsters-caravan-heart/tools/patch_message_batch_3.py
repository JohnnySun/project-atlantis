#!/usr/bin/env python3
"""Patch a bounded clean A9HJ connection-status message.

This is a third fixed-span reinsertion proof for ``g06/v00/m0006``.  It
replaces only the status text prefix, preserves the final ``FF`` byte, and
uses seven explicitly authored E1 glyph slots.  It is not the game's general
encoder; ROM and generated outputs remain local or ignored.
"""
...
MESSAGE_STRING_ID = "dqmch:a9hj:g06:v00:m0006"
MESSAGE_FILE_OFFSET = 0x286533
MESSAGE_SPAN_LENGTH = 53
MESSAGE_DATA_LENGTH = 52
MESSAGE_EXPECTED_SHA256 = "b5de7859d81d77fcc67a945dcedb1139068a26379620dff7e5df0978ef617caf"
SOURCE_HASH = "ad484c29a39bc4157b305202d8c92a45359253cb485e470573286e642a935986"
TARGET_TEXT = "連線狀態1P 確認中2P 確認中3P 確認中4P 確認中"
TARGET_TEXT_HANS = "连接状态1P 确认中2P 确认中3P 确认中4P 确认中"
PRESERVED_TAIL = bytes.fromhex("ff")
ALT_LEAD = 0xE1

# The clean extractor showed these E1 slots are unused.  The first two
# bounded batches already own the earlier slots; keep this allocation
# disjoint from both those batches as well as from the clean corpus.
GLYPH_SLOTS = {
    "連": 0xEB,
    "線": 0xEC,
    "狀": 0xED,
...
# These 8x8 patterns are authored proof glyphs, not an imported font.
GLYPH_BITMAPS = {
    "連": ("..#.....", ".#####..", "#..#.#..", "..####..", "#######.", "...#....", "..#.....", "#######."),
    "線": ("#.#.#...", ".#####..", "#.#.#...", "#######.", "..#.#...", ".#####..", "#.#.#...", "...#...."),
    "狀": ("#..#....", "#######.", "#.#.#...", ".#####..", "#.#.#...", "#######.", "#..#....", "..##...."),
...
DIRECT_CODES = {str(value): value for value in range(10)}
DIRECT_CODES["P"] = 0x19
...
def validate_bitmaps() -> dict[str, bytes]:
    ...
    if set(GLYPH_SLOTS.values()) & RESERVED_BATCH_SLOTS:
        raise ValueError("batch 3 glyph slot overlaps an earlier bounded batch")
...
def main() -> int:
    ...
    try:
        ...
        print(f"patch_message_batch_3: {error}", file=sys.stderr)
        return 2
