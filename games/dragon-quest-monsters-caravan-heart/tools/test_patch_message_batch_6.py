...
import patch_message_batch_6 as module


class MessageBatch6Test(unittest.TestCase):
    def test_reused_tiles_are_nonempty_and_unique(self) -> None:
        tiles = module.validate_bitmaps()
        self.assertEqual(set(tiles), {"開", "始", "對", "戰"})
        self.assertTrue(all(any(tile) for tile in tiles.values()))
        self.assertEqual(len(set(module.GLYPH_SLOTS.values())), 4)

    def test_target_round_trip_fits_and_preserves_ff(self) -> None:
        from verify_message_batch_6 import decode_target
    ...
    def test_allowed_ranges_cover_message_and_each_tile(self) -> None:
        from verify_message_batch_6 import allowed_ranges
