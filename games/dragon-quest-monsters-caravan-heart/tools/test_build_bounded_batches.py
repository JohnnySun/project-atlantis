...
class BoundedBatchBuildTest(unittest.TestCase):
    ...
    def test_merge_keeps_sixth_disjoint_batch_change(self) -> None:
        clean = bytes(12)
        menu = bytearray(clean)
        message = bytearray(clean)
        message_3 = bytearray(clean)
        ...
        message_6[11] = 0xBB
        self.assertEqual(
            module.merge(clean, bytes(menu), bytes(message), bytes(message_3), bytes(message_4), bytes(message_5), bytes(message_6)),
            bytes((0, 0x11, 0, 0, 0, 0, 0x66, 0, 0x88, 0x99, 0xAA, 0xBB)),
        )

    def test_merge_keeps_seventh_disjoint_batch_change(self) -> None:
        clean = bytes(13)
        menu = bytearray(clean)
        message = bytearray(clean)
        message_3 = bytearray(clean)
        ...
        message_7[12] = 0xCC
        self.assertEqual(
            module.merge(clean, bytes(menu), bytes(message), bytes(message_3), bytes(message_4), bytes(message_5), bytes(message_6), bytes(message_7)),
