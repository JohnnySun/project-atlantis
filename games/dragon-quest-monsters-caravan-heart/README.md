...
- **共用 ROM identity gate 已採用**：`scripts/gba-rom-identity.py` 以 `game.yml`／本 README 的 size、game code、CRC32、SHA-256 為 expected contract；clean ROM exit `0`／`status=pass`，被排除的 32 MiB candidate exit `1`／`status=fail`。詳細命令與報告位置記於 `research/text-cursor-contract-20260817.md`。
...
- **已固定 clean storage-form receipt**：`tools/audit_storage_form.py` 重現 8 個 group、83 個 group／variant、37,600 records、4,879 個 unique pointers 與 parser 的 direct source-byte read；目前結論是 ROM pointer pool＋mixed-byte stream，compression absence 與真正 record boundary 仍明確標成未證明，receipt 見 `research/storage-form-20260816.md`。
...
- **尚未全量翻譯**：clean 基準已具備，並已建立九筆 bounded source-free ledger（其中第八批跨 E0／E1 alternate bank）與含 alt-glyph placeholder 的本機 extractor／source-table stage；可逆抽取路徑、文字格式、完整字型／控制碼與版面仍未確認，不把 bounded proof 宣稱為全遊戲覆蓋，也不宣稱英文 patch 已被本專案覆蓋或重用。
