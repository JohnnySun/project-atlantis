...
- **尚未全量翻譯**：clean 基準已具備，並已建立四筆 bounded source-free ledger 與含 alt-glyph placeholder 的本機 extractor／source-table stage；可逆抽取路徑、文字格式、完整字型／控制碼與版面仍未確認，不把 bounded proof 宣稱為全遊戲覆蓋，也不宣稱英文 patch 已被本專案覆蓋或重用。
