...
截至 2026-08-17，已建立八個 bounded batch、共九筆 source-free ledger row；clean consumer、glyph table、mixed-byte pair
...
`message-batch-5.jsonl` 只覆蓋 clean `g06:v00:m0045` 的 two-line communication wait message。
其 encoder 保留中間 `FE` layout control 與最後 `FF`，重用前批六個 authored E1 glyph，新增 4 個
E1 glyph；restore／strip、bounded re-extraction 與 BPS apply 均已重現，仍標成 `ai_review`，
mGBA runtime QA 尚未跑。

`message-batch-6.jsonl` 只覆蓋 clean `g06:v00:m0041` 的 battle-start system line。其 encoder
重用第一批四個 authored E1 glyph、採用 clean direct `0x94` full-stop 並保留最後 `FF`；
restore／strip、bounded re-extraction 與 BPS apply 均已重現，仍標成 `ai_review`，mGBA
runtime QA 尚未跑。

`message-batch-7.jsonl` 只覆蓋 clean `g06:v00:m0040` 的 battle-start question。其 encoder
新增兩個 clean-unused E1 glyph、重用前批四個 menu glyph、採用 clean direct `0x9B` question mark
並保留最後 `FF`；restore／strip、bounded re-extraction 與 BPS apply 均已重現，仍標成
`ai_review`，mGBA runtime QA 尚未跑。

`message-batch-8.jsonl` 覆蓋相鄰的 clean `g06:v00:m0042`／`m0043` battle-decline rows。其 encoder
重用 `已`／`對`／`戰` 的 authored E1 glyph，新增 E1/FF 的 `方` 與 E0/22、E0/F7 的 `拒`／`絕`，
採用 clean direct `0x94` full-stop 並保留兩列最後 `FF`；mixed-bank bounded re-extraction 與
BPS apply 均已重現，仍標成 `ai_review`，mGBA runtime QA 尚未跑。

`tools/build_bounded_batches.py` 會從 clean ROM 分別重建八批並拒絕 range conflict；cumulative
