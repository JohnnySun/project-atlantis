...
目前已由靜態資料支持標準 Shift-JIS、`0x00` 終止和 `0x0A` 換行；另觀察到候選 `ESC C6 %s` 格式序列及 `%s`／`%d`／`%u`／`%%` 參數。M2.3 已以 controlled call 確認一條 runtime formatter→glyph cache→128-byte VRAM copy→tilemap edge，並確認 U+90E8 的 codepage identity；這不等於自然畫面 reachability、完整控制碼契約、最大寬度／行數、壓縮使用方式或回插位置。不套用黃金太陽或光明之魂的格式假設。
