...
- **M2.6 title→menu known-screen runtime slice（2026-08-16）已完成有界收尾**：以修正後
  register-specific KEYINPUT harness，clean B3EJ 與 event-system batch 2 patched ROM
  各跑一條 `settle=9s`、32-event path；兩者都從 `DISPCNT=0x1E40` 進入 `0x1F40` 的
  OAM menu state，共用 OAM renderer 讀到 24 個 visible sprites、三列主選單版面。兩次
  builder／consumer／formatter／glyph pipeline 都是 0，patched D menu 的 OAM／VRAM／
  render hash 與 clean 完全相同，因此這是 known-screen／state-change receipt 和 D
  consumer negative，不是已翻譯 menu QA 或自然 `<44` 證明；詳見
  `research/m2-6-natural-menu-runtime-20260816.md`。
- **M2.7 menu-selection negative（2026-08-16）已完成有界收尾**：START 後等待、DOWN×4、
  A×2 的第二條 32-event path 仍停在 `0x0805CF5E` title poll，沒有進入
  `0x0800C61C` normal reader 或任何 B/D/E consumer；`DISPCNT`／VRAM／OAM menu receipt
...
- **story-event E batch 8／9 已建立**：batch 8 的 E:012／E:013 與 batch 9 的 E:014／E:015
  分別覆蓋另一條結局敘事與夷陵／吳蜀衝突分支；兩批都是 2/2 layout／control／fixed-slot
  round-trip，使用 E-specific custom-aware encoder，pointer／codepage table 維持不變。
  batch 8 新增 `獻`／`亂` bounded slots；自然 E writer／VRAM receipt 與人工終審仍 pending；詳見
  `research/m3-story-event-batch8-roundtrip-20260816.md`、`research/m3-story-event-batch9-roundtrip-20260816.md`。
- **story-event E batch 10 已建立**：E:016／E:017 延續夷陵／吳蜀衝突分支；兩筆都是 2/2
  layout／control／fixed-slot round-trip，沿用 E-specific custom-aware encoder 的 `國`／`吳`／`亂`
  bounded slots，沒有新增 map slot。336-byte fixed-slot patch 與 412-byte BPS 套用逐 byte 相同；
  自然 E writer／VRAM receipt 與人工終審仍 pending，詳見
  `research/m3-story-event-batch10-roundtrip-20260816.md`。
- **story-event E batch 11 已建立**：E:018／E:019 處理漢朝復興／玉璽敘事的連續 fragment；
  兩筆都是 2/2 layout／control／fixed-slot round-trip，沿用 `獻`／`國` 的 E-specific bounded
  slots，沒有新增 map slot。210-byte fixed-slot patch 與 282-byte BPS 套用逐 byte 相同；自然
  E writer／VRAM receipt 與人工終審仍 pending，詳見
  `research/m3-story-event-batch11-roundtrip-20260816.md`。
- **story-event E batch 12 已建立**：E:020／E:021 延續漢朝衰退與劉備掌權片段；兩筆都是
...
- **story-event E batch 14 已建立**：E:024／E:025 延續漢朝復興、劉備安定生活與冷落片段；兩筆
  都是 2/2 layout／control／fixed-slot round-trip，沿用 `卻` 的 E-specific bounded slot，沒有
  新增 map slot。153-byte fixed-slot patch 與 206-byte BPS 套用逐 byte 相同；自然 E
  writer／VRAM receipt 與人工終審仍 pending，詳見 `research/m3-story-event-batch14-roundtrip-20260816.md`。
- **story-event E batch 15 已建立**：E:026／E:027 延續冷落人物刺殺與劉備信念轉折；兩筆
