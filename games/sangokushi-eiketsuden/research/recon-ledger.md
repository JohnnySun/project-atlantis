...
| M2.6 natural title→menu state | `confirmed-runtime / known-screen-bounded; consumer-negative` | clean fresh process／single connection；`DISPCNT 0x1E40→0x1F40`、BG0–BG3 維持；OAM renderer `24` visible sprites、composite PPM hash `8dd7d91c…98469a47`；VRAM before/after hash unchanged | GDB register input injection 不是 GUI／硬體 receipt；builder／B/D/E consumer、formatter、glyph、VRAM copy、tilemap 全 0；自然 index `<44` 仍 unknown |
