...
| 翻譯 ledger | `confirmed-static / bounded-ai-review` | Table B／event-system D／pool A／story-event E 共十六批、84 筆 source-free rows；各自 source hash、`zh-TW` target、上下文和 `ai_review` 均可由 ignored source table restore，十六批 strip 輸出逐 byte 相同 | pool A 尚有 115 個 unique records，E 尚有 24 筆、C 與完整劇情／武將／地名／官職／策略專名仍未建立完整批次；自然畫面 QA 尚未完成 |
