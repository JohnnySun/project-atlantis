...
class StoryLayoutAuditTest(unittest.TestCase):
    ...
    def test_batch8_ledger_and_story_map_are_bounded(self) -> None:
        ...
        self.assertEqual(by_unicode["U+6B0A"]["codepage_index"], 36)
    ...
    def test_batch10_ledger_is_source_free_and_bounded(self) -> None:
        ledger_path = pathlib.Path(__file__).parents[1] / "translations" / "story-event-batch-10.jsonl"
        rows = [json.loads(line) for line in ledger_path.read_text(encoding="utf-8").splitlines() if line.strip()]
        self.assertEqual(
            [row["string_id"] for row in rows],
            ["b3ej:story-event:016", "b3ej:story-event:017"],
        )
        self.assertTrue(all("source" not in row and row["status"] == "ai_review" for row in rows))
        self.assertEqual([row["context"]["max_width"] for row in rows], [13, 13])
        self.assertEqual([row["context"]["max_lines"] for row in rows], [4, 4])

    def test_batch11_ledger_is_source_free_and_bounded(self) -> None:
        ledger_path = pathlib.Path(__file__).parents[1] / "translations" / "story-event-batch-11.jsonl"
        rows = [json.loads(line) for line in ledger_path.read_text(encoding="utf-8").splitlines() if line.strip()]
        self.assertEqual(
            [row["string_id"] for row in rows],
            ["b3ej:story-event:018", "b3ej:story-event:019"],
        )
        self.assertTrue(all("source" not in row and row["status"] == "ai_review" for row in rows))
        self.assertEqual([row["context"]["max_width"] for row in rows], [13, 13])
        self.assertEqual([row["context"]["max_lines"] for row in rows], [3, 3])

    def test_batch12_ledger_is_source_free_and_bounded(self) -> None:
        ledger_path = pathlib.Path(__file__).parents[1] / "translations" / "story-event-batch-12.jsonl"
        rows = [json.loads(line) for line in ledger_path.read_text(encoding="utf-8").splitlines() if line.strip()]
        self.assertEqual(
            [row["string_id"] for row in rows],
            ["b3ej:story-event:020", "b3ej:story-event:021"],
        )
        self.assertTrue(all("source" not in row and row["status"] == "ai_review" for row in rows))
        self.assertEqual([row["context"]["max_width"] for row in rows], [13, 13])
        self.assertEqual([row["context"]["max_lines"] for row in rows], [3, 3])

    def test_batch13_ledger_is_source_free_and_bounded(self) -> None:
        ledger_path = pathlib.Path(__file__).parents[1] / "translations" / "story-event-batch-13.jsonl"
        rows = [json.loads(line) for line in ledger_path.read_text(encoding="utf-8").splitlines() if line.strip()]
        self.assertEqual(
            [row["string_id"] for row in rows],
            ["b3ej:story-event:022", "b3ej:story-event:023"],
        )
        self.assertTrue(all("source" not in row and row["status"] == "ai_review" for row in rows))
        self.assertEqual([row["context"]["max_width"] for row in rows], [13, 13])
    ...
    def test_batch16_ledger_is_source_free_and_bounded(self) -> None:
        ledger_path = pathlib.Path(__file__).parents[1] / "translations" / "story-event-batch-16.jsonl"
        rows = [json.loads(line) for line in ledger_path.read_text(encoding="utf-8").splitlines() if line.strip()]
        self.assertEqual(
            [row["string_id"] for row in rows],
            ["b3ej:story-event:028", "b3ej:story-event:029"],
        )
        self.assertTrue(all("source" not in row and row["status"] == "ai_review" for row in rows))
        self.assertEqual([row["context"]["max_width"] for row in rows], [13, 12])
        self.assertEqual([row["context"]["max_lines"] for row in rows], [5, 4])

    def test_batch17_ledger_is_source_free_and_bounded(self) -> None:
        ledger_path = pathlib.Path(__file__).parents[1] / "translations" / "story-event-batch-17.jsonl"
        rows = [json.loads(line) for line in ledger_path.read_text(encoding="utf-8").splitlines() if line.strip()]
        self.assertEqual(
            [row["string_id"] for row in rows],
            ["b3ej:story-event:030", "b3ej:story-event:031"],
        )
        self.assertTrue(all("source" not in row and row["status"] == "ai_review" for row in rows))
        self.assertEqual([row["context"]["max_width"] for row in rows], [12, 11])
        self.assertEqual([row["context"]["max_lines"] for row in rows], [5, 4])

    def test_batch18_ledger_is_source_free_and_bounded(self) -> None:
        ledger_path = pathlib.Path(__file__).parents[1] / "translations" / "story-event-batch-18.jsonl"
        rows = [json.loads(line) for line in ledger_path.read_text(encoding="utf-8").splitlines() if line.strip()]
        self.assertEqual(
            [row["string_id"] for row in rows],
            ["b3ej:story-event:000", "b3ej:story-event:001"],
        )
        self.assertTrue(all("source" not in row and row["status"] == "ai_review" for row in rows))
        self.assertEqual([row["context"]["max_width"] for row in rows], [13, 13])
        self.assertEqual([row["context"]["max_lines"] for row in rows], [4, 5])
