...
[`story-event-batch-17.jsonl`](story-event-batch-17.jsonl)。前者保存公開資料研究得到的
術語候選；二十七個 ledger 保存 Table B／event-system／system-item-class／story-event E 的 `zh-TW`／schema
...
E 目前共 31 個 source-free rows，仍有 2 個 unique records 待處理；公開攻略只提供
...
`strip_translations.rb` 再產生可提交 ledger。目前二十七批的 restore→strip 輸出均與 tracked
ledger 逐 byte 相同，合計 106 筆均沒有 `source` 欄位；existing-codepage rows
