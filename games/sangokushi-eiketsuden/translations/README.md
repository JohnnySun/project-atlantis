...
[`story-event-batch-12.jsonl`](story-event-batch-12.jsonl)。前者保存公開資料研究得到的
術語候選；二十二個 ledger 保存 Table B／event-system／system-item-class／story-event E 的 `zh-TW`／schema
...
custom map 重疊，所以 E batch 1／2 維持 E-specific guard；batch 3–6／8／9／10／11／12 另以完整 292-record
source-use cohort 建立十一個 custom glyph slots，不能直接沿用四池 custom-glyph patch。
E 目前共 21 個 source-free rows，仍有 12 個 unique records 待處理；公開攻略只提供
...
`strip_translations.rb` 再產生可提交 ledger。目前二十二批的 restore→strip 輸出均與 tracked
ledger 逐 byte 相同，合計 96 筆均沒有 `source` 欄位；existing-codepage rows
