...
[`story-event-batch-7.jsonl`](story-event-batch-7.jsonl)。前者保存公開資料研究得到的
術語候選；十七個 ledger 保存 Table B／event-system／system-item-class／story-event E 的 `zh-TW`／schema
...
custom map 重疊，所以 E batch 1／2 維持 E-specific guard；batch 3–6 另以完整 292-record
source-use cohort 建立九個 custom glyph slots，不能直接沿用四池 custom-glyph patch。
E 目前共 11 個 source-free rows，仍有 22 個 unique records 待處理；公開攻略只提供
...
`strip_translations.rb` 再產生可提交 ledger。目前十七批的 restore→strip 輸出均與 tracked
ledger 逐 byte 相同，合計 86 筆均沒有 `source` 欄位；existing-codepage rows
