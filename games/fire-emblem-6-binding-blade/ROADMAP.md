...
- [x] M1.26：新增 `analyze_m126_provenance.py`，以 generic caller 鄰近 `2672:16` 與 selector 鄰近 `3080:16` 的兩個不重疊 bounded windows 建立 32 筆跨 caller source/provenance census；32/32 strict worker round-trip、source-span 相等，4 筆 natural loader receipt 的 source pointer 4/4 對上、output hash 3/4 對上，index 2679 mismatch 保留且不猜原因。自然 caller families `0x08098b10`／`0x08009252` 均有收據，但 scene/content category 不從 index、caller 或 route name 推導，維持 unknown。
...
- [x] M1.31：新增 `analyze_m131_font_initializer.py`，由 `0x08098aee` → `0x08099404` 固化 `0x0837f478` ROM asset → `0x02000000` EWRAM source-base 的 initializer；source header `0x10`／expanded `0x2800`、dispatcher table row 3 `0x0809dcf5` → `SVC #0x11`、compressed length `0x1a53` + 1 byte alignment padding 均重現。對 121 個 formula inputs 做 expanded-window bounds：80 valid、41 out-of-window negative；natural 16/16 lookup 仍 valid。這證明 LZ77-WRAM initializer path，不證明 font Unicode identity、runtime source bytes 或回插。
- [x] M1.32：新增 `analyze_m132_content_gate.py`，重用 full-table strict worker census 並加入 natural loader provenance；3342 entries 中 3203 strict、139 explicit decoder-negative，table digest 固定，兩份 natural receipts 共 4 筆（source pointer 4/4、output hash 3/4，2679 mismatch）。caller families `0x08098b10`／`0x08009252` 與 indices `3087`／`2678,2679` 已分欄，但無獨立 scene/category evidence，`content_categories_assigned=[]`，不開始翻譯。
- [x] M1.33：新增 `analyze_m133_text_structure.py`，對 3342 table entries 重跑 leaf width／terminator／marker census；3203 supported records 全部 original leaf round-trip，tail `0x00` 3203/3203，two-byte units 227209，one-byte leaves 49742（opaque single-byte 26818），marker occurrences `0x00/0x01/0x04/0xff=3203/19585/136/0`。strict Shift-JIS candidate 僅 3081/3203，122 negative 以 hash 保存；arbitrary encode／marker rewrite／ROM insertion 均 disabled。
